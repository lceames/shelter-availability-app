...
def load_postal_codes():
    global _gta_postal_codes
    if _gta_postal_codes is None:
        file_path = os.path.join(app.static_folder, 'gta_postal_codes.json')
...
def get_current_records():
    ...
    if not current_records:
        ...
        update_date = YESTERDAY_DATE_STRING
    return update_date, current_records
...
def get_shelters():
    ...
    update_date, available_rooms = fetch_available_rooms()
    return jsonify({"updateData": update_date, "postalCodes": postal_codes, "shelterAvailabilities": available_rooms})
